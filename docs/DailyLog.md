...
## 260424
  - 업로드한 유저의 하드코딩을 제거하고, userId를 받아서 업로드할 수 있도록 변경
  - 팀이 존재하지 않을 때 친구목록 및 친구채팅 접속이 안되는 문제 해결
  - 친구 채팅 내 파일 삭제 로직 추가

### 코멘트
  - 별도의 오류 및 예외 처리 담당 로직과 UI가 필요. alert로 띄우는 대신, text로 알려주어야 함
  - 로딩 처리가 빠져 있음 (연속 호출을 막아야 함)
  - 팀을 친구 ID로 초대받았을 때에 대한 기능 구현이 안됨